...
##Genetic Matching ##
genout <- GenMatch(Tr = foo$Tr, X = X, estimand = "ATT", M = 1, pop.size = 300, max.generations = 30, wait.generations = 3)
summary(genout)

#exclude the Y (outcome) b/c don't want to look at until we've achieved decent balance
gen.mout <- Match(Tr = foo$Tr, X = X, estimand = "ATT", M = 1, Weight.matrix = genout, replace = TRUE, ties = TRUE)
...
genmatch.2yroutcomes <- Match(Y = Y2, Tr = foo$Tr, X = X, M = 1, estimand = "ATT", Weight.matrix = genout, replace = TRUE, ties = TRUE, BiasAdjust = TRUE)
summary(genmatch.2yroutcomes)

genmatch.5yroutcomes <- Match(Y = foo$pbs5l[valid_Y5], Tr = foo$Tr[valid_Y5], X = X[valid_Y5,], M = 1, estimand = "ATT", Weight.matrix = genout, replace = TRUE, ties = TRUE, BiasAdjust = TRUE)
summary(genmatch.5yroutcomes)

##Combine all meaningful results into a table (the three methods required in hw, and the GenMatch w/ propensity score extra)
...
##The leximan p value < 0.1, so include neither treatment affects 
results <- rbind(results, c(NA, NA, 2.22e-16))
results <- rbind(results, c(NA, NA, 2.22e-16))

## And then the GenMatch score results..                        
results <- rbind(results, c(genmatch.2yroutcomes$est, genmatch.2yroutcomes$est.noadj, 0.31745))
results <- rbind(results, c(genmatch.5yroutcomes$est, genmatch.5yroutcomes$est.noadj, 0.31745))

#Round results because the numbers for these columns were long/cluttered the table
results[,1:2] <- round(results[,1:2], 2)
